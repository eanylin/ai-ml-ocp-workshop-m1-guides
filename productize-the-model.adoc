...
[WARNING] 
====
Copying the file over would change the contents of the lr.py file, but since this is a prepared file the contents replacing the original lr.py file would be accurate. Users can view the new contents of the file via the file explorer as stated previously.
====
