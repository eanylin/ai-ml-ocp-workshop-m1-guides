...
link:%7B%7B%20CONSOLE_URL%20%7D%7D[\{\{ CONSOLE_URL }}]. The login page
prompts user for their Username and Password.

Your user name will be `{{  USER_ID }}` and password is
`{{  OPENSHIFT_USER_PASSWORD }}`

image:%7B%%20image_path%20openshift-ui-login.png%20%%7D[openshift_ui_login]

We will be presented with a `Welcome to OpenShift` message and the
option of creating a new project

image:%7B%%20image_path%20welcome-to-openshift.png%20%%7D[welcome_to_openshift]

== Creating a New Project

Proceed to create a new project by selecting `Create Project`

image:%7B%%20image_path%20openshift-create-new-project.png%20%%7D[openshift_create_new_project]
...
image:%7B%%20image_path%20openshift-project-list.png%20%%7D[openshift_project_list]
...
image:%7B%%20image_path%20myproject-overview-page.png%20%%7D[myproject_overview_page]
...
image:%7B%%20image_path%20openshift-developer-view.png%20%%7D[openshift_developer_view]
...
image:%7B%%20image_path%20openshift-developer-catalog.png%20%%7D[openshift_developer_catalog]
...
image:%7B%%20image_path%20developer-catalog-python.png%20%%7D[developer_catalog_python]

This will bring up a dialog with the details of the builder image. Click
on `Create Application` in the dialog.

image:%7B%%20image_path%20python-s2i-create-app.png%20%%7D[python_s2i_create_app]
...
image:%7B%%20image_path%20django-git-repo-url.png%20%%7D[django_git_repo_url]
...
image:%7B%%20image_path%20django-application-settings.png%20%%7D[django_application_settings]
...
image:%7B%%20image_path%20django-application-topology-view.png%20%%7D[django_application_topology_view]
...
image:%7B%%20image_path%20django-image-build.png%20%%7D[django_image_build]

image:%7B%%20image_path%20django-image-build-logs.png%20%%7D[django_image_build_logs]

Once the build of the application image has completed, it will be
deployed.

image:%7B%%20image_path%20django-app-deployment.png%20%%7D[django_app_deployment]
...
image:%7B%%20image_path%20django-app-deployed.png%20%%7D[django_app_deployed]

Clicking on the URL link will bring us to the Django application that
was just deployed.

image:%7B%%20image_path%20django-app-webpage.png%20%%7D[django_app_webpage]
...
image:%7B%%20image_path%20openshift-administrator-view.png%20%%7D[openshift_administrator_view]
...
image:%7B%%20image_path%20django-pods.png%20%%7D[django_pods]
...
image:%7B%%20image_path%20django-services.png%20%%7D[django_services]
...
image:%7B%%20image_path%20django-routes.png%20%%7D[django_routes]
...
image:%7B%%20image_path%20django-deployment-configs.png%20%%7D[django_deployment_configs]

image:%7B%%20image_path%20django-deployment-configs-yaml.png%20%%7D[django_deployment_configs_yaml]
...
image:%7B%%20image_path%20django-replication-controllers.png%20%%7D[django_replication_controllers]

image:%7B%%20image_path%20django-replication-controllers-overview.png%20%%7D[django_replication_controllers_overview]
...
image:%7B%%20image_path%20django-secrets.png%20%%7D[django_secrets]
...
image:%7B%%20image_path%20django-configmaps.png%20%%7D[django_configmaps]
...
image:%7B%%20image_path%20openshift-pv-pvc-sample.png%20%%7D[openshift_pv_pvc_sample]
