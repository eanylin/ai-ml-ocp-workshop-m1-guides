...
`train.sh` has been written to be used in the pipeline to call your python script and saves the model into the pipeline workspace. `train.sh` can be found in your {{GIT_URL}}/{{USER_ID}}/datascience-examples/[Gogs^] repository. 
