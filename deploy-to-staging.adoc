...
The source code is available {{GIT_URL}}/{{USER_ID}}/{{USER_MODEL_REPO_NAME}}/src/staging/workshop/src/seldon[here] 
...
Once the pipeline runs finish,the image would have been pushed into OpenShift {{CONSOLE_URL}}/k8s/ns/{{USER_ID}}-stage/imagestreams[registry] and is tagged with the git revision number. This allows us to provide model provenance by tracking the source code, data version used and the image being used.
...
git checkout -b stage
sed -e "s/_USER_/{{USER_ID}}/g" -e "s/_ENV_/stage/g" -e "s/_IMAGE_NAME_/$IMAGE_NAME/g" -e "s/_GIT_REV_/$GIT_REV/g" seldon-model.yaml.tmpl > seldon.yaml
----
...
spec:
  containers:
    - image: image-registry.openshift-image-registry.svc:5000/{{USER_ID}}-stage/lr:xxxxxxx
----
...
{{ARGOCD_URL}}[Argo CD^] is configured to monitor your deployment for the `stage` and `prod` branch in your git 
...
A `Deployment` resource will be created. The pods should be running and in ready state.
...
[source,sh,role="copypaste"]
----
/projects/{{USER_MODEL_REPO_NAME}}/workshop/bin/stage-test.sh
----
