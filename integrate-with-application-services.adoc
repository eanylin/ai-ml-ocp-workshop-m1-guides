...
. The consumer application will only route any fraud outcome to a Web application to display the results so that users will be able to view.

[NOTE]
====
Since the consumer application only routes fraud outcomes to the Web application, any outcome determined to be a non-fraud will not be displayed.
This is reflective of a real world scenario where users receive notifications when a suspected fraud case might have occured in their accounts.
====
...
image::decision-flow-chart.png[decision-flow-chart, 400w]
